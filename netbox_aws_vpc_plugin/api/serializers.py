...
class AWSVPCSerializer(NetBoxModelSerializer):
    ...
    class Meta:
        model = AWSVPC
        fields = (
            'id', 'url', 'display', 'vpc_id', 'name', 'arn', 'vpc_cidr', 'owner_account',
...
class AWSSubnetSerializer(NetBoxModelSerializer):
    ...
    class Meta:
        model = AWSSubnet
        fields = (
            'id', 'url', 'display', 'subnet_id', 'name', 'arn', 'subnet_cidr', 'vpc',
            'owner_account', 'tags', 'custom_fields', 'created', 'last_updated',
        )


class AWSAccountSerializer(NetBoxModelSerializer):
    url = serializers.HyperlinkedIdentityField(
        view_name='plugins-api:netbox_aws_vpc_plugin-api:awsaccount-detail'
    )

    class Meta:
        model = AWSAccount
        fields = (
            'id', 'url', 'display', 'account_id', 'name', 'arn', 'description',
            'tags', 'custom_fields', 'created', 'last_updated',
        )
