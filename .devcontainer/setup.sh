...
for i in {1..30}; do
    if DJANGO_SETTINGS_MODULE=netbox.settings SECRET_KEY="${SECRET_KEY:-dummydummydummydummydummydummydummydummydummydummydummydummy}" DEBUG="${DEBUG:-True} " API_TOKEN_PEPPER_1="${API_TOKEN_PEPPER_1:-abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456}" python -c "
import django
django.setup()
...
# Ensure API_TOKEN_PEPPER_1 is set for migration
...
DEBUG="${DEBUG:-True}" SECRET_KEY="${SECRET_KEY:-dummydummydummydummydummydummydummydummydummydummydummydummy}" API_TOKEN_PEPPER_1="${API_TOKEN_PEPPER_1:-abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456}" \
...
alias netbox-test="cd /opt/netbox/netbox && source /opt/netbox/venv/bin/activate && python manage.py test netbox_aws_vpc_plugin.tests --parallel -v2"
